...
| 测试 | 验证 |
|---|---|
| `test_version` | `distify --version` 正常退出且输出包含 "distify" |
| `test_help_lists_commands` | 四个命令 + frameworks 都注册到了帮助 |
| `test_frameworks_command_lists_ray_and_aura` | `distify frameworks` 列出 ray 和 aura |
...
| `test_cli_analyze_and_convert` | 子进程启动 `python -m distify.cli`，真实 CLI 调用 |
...
- **性能断言**：测试没有硬性断言"加速比 > X×"，只验证正确性。小数据上 Ray 冷启反而比原版慢，这是预期。加速比验证放在 `distify verify` 命令里做，而不是单元测试。
