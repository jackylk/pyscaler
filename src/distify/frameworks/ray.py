...
class RayFramework(Framework):
    ...
    def _rewrite_dataframe_apply(tree: ast.Module, info: dict, workers: int) -> None:
        ...
        # Inject a _distify_apply_chunk helper at module top (after imports)
        helper_src = textwrap.dedent(f"""
            @ray.remote
            def _distify_apply_chunk(chunk):
        ...
        replacement_src = textwrap.dedent(f"""
            ...
            _results = ray.get([_distify_apply_chunk.remote(c) for c in _chunks])
